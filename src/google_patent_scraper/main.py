...
class scraper_class:
    ...
    def process_patent_html(self,soup):
        ...
        try: 
            patent_full_url = soup.find('a',itemprop="pdfLink").attrs['href']
        except:
            patent_full_url = ''
            
        # Title
        try: 
            title = soup.find('span',itemprop="title").contents[0].strip()
        except:
            title = ''
        ...
        list_of_classification_codes = soup.find_all('span',itemprop='Code')
        list_of_classification_descs = soup.find_all('span',itemprop='Description')
        classification_code_descss=[]
        p_code=""
        classification_code_descs=[]
        for i in range(len(list_of_classification_codes)):
            classification_code=list_of_classification_codes[i].get_text()
            classification_desc=list_of_classification_descs[i].get_text()
            if not classification_code.startswith(p_code):
                classification_code_descss.append(classification_code_descs)
                classification_code_descs=[]
            p_code=classification_code
            classification_code_descs.append((classification_code, classification_desc))
        ...
        #  Get text 
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
        list_of_descs = soup.find_all("div", id=re.compile("p.*"))
        patent_text=""
        for desc in list_of_descs:
            patent_text+=desc.text+"\n"
        
        # Claims
        list_of_descs = soup.find_all("div", attrs={"class": re.compile("claim-text.*")})
        for desc in list_of_descs:
            patent_text+=desc.text+"\n"
        
        list_of_descs = soup.find_all("claim-text")
        for desc in list_of_descs:
            patent_text+=desc.text+"\n"
        ...
        return({'inventor_name':json.dumps(inventor_name),
                ...
                'patent_full_url': patent_full_url,
                'title': title,
                'classification_code_descss': classification_code_descss,
